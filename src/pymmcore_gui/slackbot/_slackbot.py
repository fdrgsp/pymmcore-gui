...
class SlackBot:
    ...
    def __init__(self) -> None:
        ...
        @self._app.command("/image")  # type: ignore [misc]
        def handle_image_commands(ack: Ack, body: dict) -> None:
            ack()
            self._forward_command(body.get("command", ""))
        ...
        while True:
            if message := sys.stdin.readline().strip():
                try:
                    ...
                    logging.warning(
                        "SlackBot -> Message is not valid JSON; sending as plain text."
                    )
                    self.send_message(message)

    def _handle_image_array(self, message: dict[str, str]) -> None:
        """Handles a base64-encoded NumPy array and uploads it as an image."""
        data = message.get("data")
        text = message.get("text", "Image from process")

        if not data:
            logging.error("SlackBot -> No image data received.")
            return
        ...
        try:
            ...
            if len(img.shape) != 2:
                logging.error(
                    "SlackBot -> Image array is not 2D; cannot upload to Slack."
                )
                return
            ...
            logging.info("SlackBot -> Image uploaded successfully.")

        except Exception as e:
            logging.error(f"SlackBot -> Failed to handle image array: {e}")
