"""Test script for NGFFWriterWrapper with TIFF backend."""
...
# Test with tiff backend
backend = "tiff"
suffix = ".ome.tiff"
tmp_dir = Path(f"/tmp/test_data{suffix}")

# Single position to avoid the TIFF metadata bug
seq = useq.MDASequence(channels=["DAPI", "FITC"])
...
print("\n" + "=" * 60)
print("Testing NGFFWriterWrapper with TIFF")
print("=" * 60)
...
print(f"\nFirst channel shape: {data_slice.shape}")
print(f"Data type: {data_slice.dtype}")
print(f"Min: {data_slice.min()}, Max: {data_slice.max()}")

# Get all data
data_slice = wrapper.isel({})
print(f"\nAll data shape: {data_slice.shape}")
